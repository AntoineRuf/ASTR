    ...
    class QuickDash : Skill
    {
        ...
        public override void Apply(Unit caster, List<Cell> cells, CellGrid cellGrid)
        {
            foreach (var currentCell in cells)
            {
                ...
                Vector3 direction = cellCubeCoord - casterCubeCoord;
                ...
                for( int i = 0; i<= 5; i++)
                {
                    ...
                    Cell cellToAdd = cellGrid.Cells.Find(c => c.OffsetCoord == CellAffectedOffsetCoord && !c.IsTaken);
                    if (cellToAdd != null)
                    {
                ...
                Cell destination = affectedCells.Find(c => c.GetDistance(caster.Cell) == 5);
                caster.Cell = destination;
                caster.transform.position = destination.transform.position;
            }
